    ...
    (results) => {
      ...
      if (data.length === 0) {
        ...
        output.innerHTML = data
          .map(
            (product, i) =>
              `<div class="flex flex-col border mx-6 mb-2 p-4 rounded-lg shadow-lg">
...
async function scrapeProducts() {
  ...
  const products = cards.map((el) => {
    ...
    const priceTag =
      el
        .querySelector(
          "#undefined-46 > div > div > div > div.showcase-description > div.Showcase__details > div.Showcase__details__text > div > div > div > div.Showcase__priceTag.Showcase__priceTag--salesPrice > span"
        )
        ?.textContent.trim() || "-0%";
    return { name, sellerName, salePrice, priceTag }; // Retorna un objeto por cada producto
  });
